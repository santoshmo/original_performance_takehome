...
class KernelBuilder:
    ...
    def build_simd_ir_kernel(
        ...
        simd_hash_scalar_by_round = {
            ...
            for round_i, policy in variant.get("simd_hash_scalar_by_round", {}).items()
        }
        ...
        simd_predrain_group_order = variant.get("simd_predrain_group_order", "forward")
        simd_predrain_group_sequence = variant.get("simd_predrain_group_sequence")
        simd_predrain_group_rotation = variant.get("simd_predrain_group_rotation", 0)
        ...
        for gi in range(group_size):
            contexts.append(
                {
                    "node": self.alloc_vec(f"node_tmp_{gi}", "node_tmp", pool="context"),
                    "tmp1": self.alloc_vec(f"hash_tmp_{gi}", "hash_tmp", pool="context"),
                    "tmp2": self.alloc_vec(f"select_tmp0_{gi}", "select_tmp", pool="context"),
                    "tmp3": self.alloc_vec(f"select_tmp1_{gi}", "select_tmp", pool="context"),
                    "store_addr": (
                        self.alloc_role(
                            f"store_addr_{gi}",
                            "addr_tmp",
                                ...
                                if mul_vec is not None:
                                    ...
                                    if h2_scalar:
                                        ...
                                        for lane in range(VLEN):
                                            emit_simd_op(
                                                "alu",
                                                (op3, ctx["tmp2"] + lane, val_vec + lane, c3),
                                                tag="hash",
                                                round_i=round_i,
                                                group=block,
                                                contributes_value=True,
                                    ...
                                    if combine_scalar:
                                        for lane in range(VLEN):
                                            emit_simd_op(
                                                "alu",
                                                (op2, val_vec + lane, ctx["tmp1"] + lane, ctx["tmp2"] + lane),
                                                tag="hash",
                                                round_i=round_i,
                                                group=block,
                                                contributes_value=True,
                ...
                for gi in gi_order:
                    ...
                    for round_i in range(round_start, round_end):
                        ...
                        if level == 0:
                            ...
                            add("flow", ("vselect", ctx["tmp1"], ctx["tmp2"], node_vecs[4], node_vecs[3]), "select")
                            add("flow", ("vselect", ctx["tmp2"], ctx["tmp2"], node_vecs[6], node_vecs[5]), "select")
                            add("flow", ("vselect", ctx["node"], ctx["node"], ctx["tmp2"], ctx["tmp1"]), "select")
                            emit_xor(ctx["node"])
                        elif level == 3:
                            add("valu", ("&", ctx["tmp2"], idx_vec, one_vec), "select")
                            add("valu", ("&", ctx["tmp3"], idx_vec, two_vec), "select")
                            add("flow", ("vselect", ctx["node"], ctx["tmp2"], node_vecs[8], node_vecs[7]), "select")
                            add("flow", ("vselect", ctx["tmp1"], ctx["tmp2"], node_vecs[10], node_vecs[9]), "select")
                            add("flow", ("vselect", ctx["tmp1"], ctx["tmp3"], ctx["tmp1"], ctx["node"]), "select")
                            add("flow", ("vselect", ctx["node"], ctx["tmp2"], node_vecs[12], node_vecs[11]), "select")
                            add("flow", ("vselect", ctx["tmp2"], ctx["tmp2"], node_vecs[14], node_vecs[13]), "select")
                            add("flow", ("vselect", ctx["node"], ctx["tmp3"], ctx["tmp2"], ctx["node"]), "select")
                            add("valu", ("&", ctx["tmp2"], idx_vec, four_vec), "select")
                            add("flow", ("vselect", ctx["node"], ctx["tmp2"], ctx["node"], ctx["tmp1"]), "select")
                            emit_xor(ctx["node"])
                        else:
                            addr_base_vec = level_base_vecs[level]
                            for lane in range(VLEN):
                        ...
                        for hi, (op1, _val1, op2, op3, _val3) in enumerate(HASH_STAGES):
                            ...
                            if mul_vec is not None:
                                ...
                                if h2_scalar:
                                    c3 = scalar_const(HASH_STAGES[hi][4])
                                    for lane in range(VLEN):
                                        add("alu", (op3, ctx["tmp2"] + lane, val_vec + lane, c3), "hash")
                                else:
                                    add("valu", (op3, ctx["tmp2"], val_vec, hash_vec_consts3[hi]), "hash")
                                if combine_scalar:
                                    for lane in range(VLEN):
                                        add("alu", (op2, val_vec + lane, ctx["tmp1"] + lane, ctx["tmp2"] + lane), "hash")
                                else:
                                    add("valu", (op2, val_vec, ctx["tmp1"], ctx["tmp2"]), "hash")
                        ...
                        if simd_drop_final_index and round_i == rounds - 1:
                            ...
                            for lane in range(VLEN):
                                add("alu", ("&", ctx["tmp1"] + lane, val_vec + lane, one_const), "index")
                            if level in scalar_index_levels or round_i in scalar_index_rounds:
                                for lane in range(VLEN):
                                    add("alu", ("<<", idx_vec + lane, idx_vec + lane, one_const), "index")
                                    add("alu", ("+", idx_vec + lane, idx_vec + lane, ctx["tmp1"] + lane), "index")
                            else:
                                add("valu", ("multiply_add", idx_vec, idx_vec, two_vec, ctx["tmp1"]), "index")
