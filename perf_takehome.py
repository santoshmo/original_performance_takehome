...
class KernelBuilder:
    ...
    def build_simd_ir_kernel(
        ...
        simd_predrain_group_rotation = variant.get("simd_predrain_group_rotation", 13)
        simd_predrain_hash_rounds = {
            int(round_i) for round_i in variant.get("simd_predrain_hash_rounds", (13,))
        }
        simd_selector_ring_depth = variant.get("simd_selector_ring_depth", 1)
                ...
                for gi in gi_order:
                    ...
                    for round_i in range(round_start, round_end):
                        ...
                        if level == 0:
                            ...
                            add("flow", ("vselect", ctx["tmp2"], low_bit, node_vecs[6], node_vecs[5]), "select")
                            add("flow", ("vselect", ctx["node"], high_bit, ctx["tmp2"], ctx["tmp1"]), "select")
                            emit_xor(ctx["node"])
                        elif level == 3:
                            low_bit = selector_bit(1)
                            ...
                            add("flow", ("vselect", ctx["tmp2"], low_bit, node_vecs[14], node_vecs[13]), "select")
                            add("flow", ("vselect", ctx["node"], mid_bit, ctx["tmp2"], ctx["node"]), "select")
                            if high_bit is None:
                                high_bit = ctx["tmp2"]
