...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        scalar_xor_levels = set(
            variant.get("scalar_xor_levels", (0, 1, 3, 4, 5, 6, 7, 8, 9, 10))
        )
        scalar_hash_stages = set(variant.get("scalar_hash_stages", ()))
        scalar_hash_depths = set(variant.get("scalar_hash_depths", range(forest_height + 1)))
                        ...
                        if level == forest_height:
                            ...
                            if level in scalar_index_levels:
                                for lane in range(VLEN):
                                    ...
                                    slots.append(
                                        ("alu", ("+", idx_vec + lane, idx_vec + lane, ctx["tmp1"] + lane))
                                    )
                            else:
                                slots.append(
        ...
        scheduled = _schedule_slots(slots)
