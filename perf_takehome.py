...
class KernelBuilder:
    ...
    def _schedule_ir_asap(
        ...
        self.ir_op_cycles = compact_op_cycle
    ...
    def _reschedule_tail_window(
        ...
        pred_closure: set[int] = set()
        if order == "final_chain":
            rounds = [op.round for op in ops if op.round is not None]
            final_round = max(rounds) if rounds else None
            final_groups = [
                op.group
                ...
                and op.round == final_round
                and op.group is not None
            ]
            target_group = weights.get(
                "tail_reschedule_target_group",
                max(final_groups) if final_groups else None,
            )
            target_ops = {
                op_id
                for op_id, op in enumerate(ops)
                if op_id in tail_ops
                and op.round == final_round
                and op.group == target_group
                and op.tag in ("gather", "hash", "store")
                and op.contributes_value
            }
            # Include final gather ops too; they do not set contributes_value.
            target_ops.update(
                op_id
                for op_id, op in enumerate(ops)
                if op_id in tail_ops
            ...
            stack = list(target_ops)
            while stack:
                op_id = stack.pop()
                if op_id in pred_closure:
                    continue
                pred_closure.add(op_id)
                for pred, _latency in pred_edges[op_id]:
                    if pred in tail_ops and pred not in pred_closure:
                        stack.append(pred)
        ...
        def priority(op_id: int) -> tuple[int, int]:
            ...
            if order == "final_chain":
                focus_bonus = 5000 if op_id in pred_closure else 0
                hash_bonus = 500 if ops[op_id].tag == "hash" else 0
                gather_bonus = 300 if ops[op_id].tag == "gather" else 0
                store_bonus = 100 if ops[op_id].tag == "store" else 0
                score = (
                    focus_bonus
                    + hash_bonus
                    + gather_bonus
                    + store_bonus
                    + critical_height[op_id]
                    + tail_score[op_id]
                )
                return (-score, op_id)
    ...
    def build_simd_ir_kernel(
        ...
        simd_final_gather_group_order = variant.get(
            "simd_final_gather_group_order", simd_final_group_order
        )
        simd_final_gather_group_sequence = variant.get("simd_final_gather_group_sequence")
        simd_final_gather_group_rotation = variant.get(
            "simd_final_gather_group_rotation", simd_final_group_rotation
        )
        ...
        simd_final_gather_prepass = variant.get("simd_final_gather_prepass", False)
        simd_interleave_final_stores = variant.get("simd_interleave_final_stores", False)
        simd_store_tail_address_mode = variant.get("simd_store_tail_address_mode", "offset_chain")
        ...
        for group_start, round_start, round_end, is_final_drain in tile_plan():
                if (
                    ...
                    if level >= 4:
                        ...
                        gather_groups = []
                        for gi in ordered_group_indices(
                            simd_final_gather_group_order,
                            simd_final_gather_group_sequence,
                            simd_final_gather_group_rotation,
                            label="simd_final_gather_group_sequence",
                        ):
                            block = group_start + gi
                            if block >= blocks_per_round:
                                break
                            gather_groups.append((block, contexts[gi]))

                        addr_base_vec = level_base_vecs[level]
                        for block, ctx in gather_groups:
                        ...
                        for block, ctx in final_groups:
                            ...
                            def add_final(engine: str, slot: tuple, tag: str = "hash"):
                                emit_simd_op(
                                    engine,
                                    slot,
                                    tag=tag,
                                    round_i=round_i,
                                    group=block,
                                    contributes_value=tag == "hash",
                                    contributes_index=tag == "index",
                                    droppable_final=tag == "index",
                                )
                            ...
                            if level in scalar_xor_levels:
                                ...
                                add_final("valu", ("^", val_vec, val_vec, ctx["node"]), "hash")
                            ...
                            for hi, (op1, _val1, op2, op3, _val3) in enumerate(HASH_STAGES):
                                ...
                                if h1_scalar:
                                    ...
                                    add_final("valu", (op1, ctx["tmp1"], val_vec, hash_vec_consts1[hi]), "hash")
                                if h2_scalar:
                                    ...
                                    add_final("valu", (op3, h2_tmp, val_vec, hash_vec_consts3[hi]), "hash")
                                if combine_scalar:
                                    for lane in range(VLEN):
                                        add_final(
                                            "alu",
                                    ...
                                    add_final("valu", (op2, val_vec, ctx["tmp1"], h2_tmp), "hash")
        ...
        for block in range(blocks_per_round):
            if block in interleaved_store_blocks:
                continue
            addr_reg = tmp_addr if block % 2 == 0 else tmp_addr2
            if simd_store_tail_address_mode == "offset_chain":
                addr_reg = tmp_addr
                emit_simd_op(
                    "alu",
                    ("+", addr_reg, self.scratch["inp_values_p"], offset),
                    tag="store",
                    round_i=rounds - 1,
                    group=block,
                    contributes_value=True,
                )
            elif simd_store_tail_address_mode == "load_const":
                emit_simd_op(
                    "load",
                    ("const", addr_reg, block * VLEN),
                    tag="store",
                    round_i=rounds - 1,
                    group=block,
                    contributes_value=True,
                ...
                emit_simd_op(
                    "alu",
                    ("+", addr_reg, self.scratch["inp_values_p"], addr_reg),
                    tag="store",
                    round_i=rounds - 1,
                    group=block,
                    contributes_value=True,
                )
            elif simd_store_tail_address_mode == "add_imm":
                emit_simd_op(
                    "flow",
                    ("add_imm", addr_reg, self.scratch["inp_values_p"], block * VLEN),
                    tag="store",
                    round_i=rounds - 1,
                    group=block,
                    contributes_value=True,
                ...
                raise ValueError(f"Unknown store-tail address mode {simd_store_tail_address_mode!r}")
            emit_simd_op(
                "store",
                ("vstore", addr_reg, val_base + block * VLEN),
                tag="store",
                round_i=rounds - 1,
                group=block,
                contributes_value=True,
